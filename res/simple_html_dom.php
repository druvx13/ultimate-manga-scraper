...
class ums_simple_html_dom
{
	...
	protected function prepare($str, $lowercase=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new ums_simple_html_dom_node($this);
		$this->root->tag = 'root';
	...
	protected function parse()
	{
		while (true) {
			...
			$node = new ums_simple_html_dom_node($this);
			++$this->cursor;
	 ...
	 *
	 * Noise is stored to {@see ums_simple_html_dom::$noise}
	 *
	 ...
	 *
	 * Noise is restored from {@see ums_simple_html_dom::$noise}
	 *
	...
	function createElement($name, $value=null) {return ums_str_get_html("<$name>$value</$name>")->first_child();}
	function createTextNode($value) {return end(ums_str_get_html($value)->nodes);}
	function getElementById($id) {return $this->find("#$id", 0);}
	function getElementsById($id, $idx=null) {return $this->find("#$id", $idx);}
	function getElementByTagName($name) {return $this->find($name, 0);}
